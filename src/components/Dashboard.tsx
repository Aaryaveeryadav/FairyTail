...
export function Dashboard() {
  ...
  const [duration, setDuration] = useState(() =>
    couple?.start_date
      ? calcDuration(couple.start_date)
      : null,
  );
  ...
  return (
    <div className="space-y-6 pb-4 animate-fade-in">
      ...
      <div className="p-5 rounded-2xl bg-white border border-cream-300 shadow-sm">
        ...
        <p className="font-serif text-lg text-ink-900 mb-3 text-balance">
          {todayQuestion}
        </p>

        <p className="text-xs text-ink-400">
          Answer together in the Love tab
        </p>
      ...
      {showMoodPicker && (
        <div
          className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-ink-900/40 backdrop-blur-sm animate-fade-in"
          onClick={() =>
            setShowMoodPicker(false)
          }
        >
          ...
          <div
            className="w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-3xl p-6 animate-slide-up"
            onClick={(e) =>
              e.stopPropagation()
            }
          >
              ...
              {MOODS.map((mood) => (
                <button
                  ...
                  className={`p-3 rounded-2xl border transition-all hover:scale-105 ${
                    selectedMood ===
                    mood.type
                      ? `${mood.color} ring-2 ring-rose-300`
